...
All data is dynamically fetched via REST API and parsed natively in PySpark.
* **Source:** Alpha Vantage API
* **Endpoints Used:** `TOP_GAINERS_LOSERS`, `TIME_SERIES_DAILY`, `NEWS_SENTIMENT`, `OVERVIEW`
* **Update Frequency:** Daily automated batch runs.
...
* **Dynamic Company Profiling:** Dashboard titles, sector/industry subtitles, and full business descriptions automatically update based on the selected ticker symbol.
* **Custom KPI Modules:** Engineered DAX measures dynamically track the latest close price and daily volume against 100-day moving averages, triggering custom text indicators (🔺 Above Average / 🔻 Below Average) and conditional formatting.
* **Web Integration:** Formatted data categories natively convert raw URL text into clickable web icons for seamless navigation to company sites.
